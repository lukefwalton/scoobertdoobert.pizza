...
const paused = await page.waitForSelector('.hud-pause', { timeout: 6000 }).then(
  () => true,
  () => false,
);
  ...
  .waitForFunction(
    () => {
      const b = [...document.querySelectorAll('button')].find(
        (b) => b.getAttribute('aria-label') === 'next song',
      );
      return !!b && !b.disabled;
    },
    null,
...
if (!/\/audio\/jukebox\/.+\.mp3$/.test(url))
  bad(`switcher track url looks wrong: ${JSON.stringify(url)}`);
