...
await page.waitForSelector('.hud-menu-btn', { timeout: 12000 }).catch(() => bad('jukebox never mounted'));
...
const rolled = await (async () => {
  for (const [fx, fy] of [
    [0.664, 0.606], [0.65, 0.6], [0.68, 0.61], [0.66, 0.62], [0.67, 0.59], [0.655, 0.615], [0.675, 0.6],
  ]) {
...
await page.keyboard.press('Escape');
await page.waitForSelector('.hud-pause', { timeout: 6000 }).catch(() => bad('pause menu did not open (post-unlock)'));
const flipReady = await page
  .waitForFunction(
    () => {
      const b = [...document.querySelectorAll('button')].find((x) => x.getAttribute('aria-label') === 'next song');
      return !!b && !b.disabled;
    },
    null,
    { timeout: 12000 },
...
if (flipReady) {
  ...
  flipped = await page
    .waitForFunction((prev) => !!window.__sdpJukeboxUrl && window.__sdpJukeboxUrl !== prev, urlBefore, {
      timeout: 12000,
    })
    .then(() => true, () => false);
