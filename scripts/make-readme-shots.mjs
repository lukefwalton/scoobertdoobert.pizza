...
let promotedCount = 0;
// try/finally so a thrown goto/click/selector error still closes Chromium and
// removes the staging dir — no orphaned browser process or temp dir on a crash.
try {
  ...
  // Clean run only: promote the staged shots into the committed media folder. Every
  // file is known-good (we got here with zero failures), so this can't publish a
  // half-captured set.
  if (!errors) {
...
console.log(`readme shots done — promoted ${promotedCount} file(s) into ${OUT}.`);
